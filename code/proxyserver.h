...
struct parsed_request * parse_client_request(int fd) {
    ...
    request -> read_buffer = malloc(LIBHTTP_REQUEST_MAX_SIZE + 1);
    strcpy(request -> read_buffer, read_buffer);


    if (strstr(read_buffer, GETJOBCMD) != NULL) {
        //this means that it was a getJob request
        return NULL;
    }
